...
from sfa.unigetestbed.unigetestbedshell import unigetestbedShell
from sfa.unigetestbed.unigetestbedslices import unigetestbedSlices
from sfa.unigetestbed.unigetestbedaggregate import unigetestbedAggregate
...
from sfa.unigetestbed.unigetestbedxrn import unigetestbedXrn, slicename_to_hrn, hostname_to_hrn, hrn_to_unigetestbed_slicename, xrn_to_hostname

nodes_list = []
slices = []
indexes = {'nodes_index':1,'slices_index':1}
...
class unigetestbedDriver (Driver):
    ...
    def check_sliver_credentials(self, creds, urns):
        ...
        for cred in creds:
            ...
            slice_cred_names.append(unigetestbedXrn(xrn=slice_cred_hrn).unigetestbed_slicename())
    ...
    def register (self, sfa_record, hrn, pub_key):
        ...
        unigetestbed_record = self.sfa_fields_to_unigetestbed_fields(type, hrn, sfa_record)

        if type == 'authority':
            pointer = -1

        elif type == 'slice':
            slices = self.shell.GetSlices({'slice_name': unigetestbed_record['slice_name']})
            if not slices:
                pointer = self.shell.AddSlice(unigetestbed_record)
            else:
            ...
            if not users:
                pointer = self.shell.AddUser(unigetestbed_record)
            else:
            ...
            nodes = self.shell.GetNodes(unigetestbed_record['hostname'])
            if not nodes:
                pointer = self.shell.AddNode(unigetestbed_record)
            else:
    ...
    def update (self, old_sfa_record, new_sfa_record, hrn, new_key):
        ...
        unigetestbed_record=self.sfa_fields_to_unigetestbed_fields(type, hrn, new_sfa_record)
        ...
        if type == "slice":
            self.shell.UpdateSlice({'slice_id': pointer, 'fields': unigetestbed_record})

        elif type == "user":
            self.shell.UpdateUser({'user_id': pointer, 'fields': unigetestbed_record})

            if new_key:
                self.shell.AddUserKey({'user_id' : pointer, 'key' : new_key})

        elif type == "node":
            self.shell.UpdateNode({'node_id': pointer, 'fields': unigetestbed_record})
    ...
    ##
    # Convert SFA fields to unigetestbed testbed fields for use when registering or updating
    # registry record in the unigetestbed testbed
    #
    ...
    def fill_record_info(self, records):
        ...
        self.fill_record_unigetestbed_info(records)
        self.fill_record_hrns(records)
        self.fill_record_sfa_info(records)
        return records

    def fill_record_unigetestbed_info(self, records):
        """
        ...
        unigetestbed_records = {'node': nodes, 'slice': slices, 'user': users}


        # fill record info
        for record in records:
            # records with pointer==-1 do not have unigetestbed info.
            if record['pointer'] == -1:
                continue

            for type in unigetestbed_records:
                if record['type'] == type:
                    if record['pointer'] in unigetestbed_records[type]:
                        record.update(unigetestbed_records[type][record['pointer']])
                        break
            # fill in key info
            if record['type'] == 'user':
                record['key_ids'] = []
                record['keys'] = []
                for key in unigetestbed_records['user'][record['pointer']]['keys']:
    ...
    def fill_record_hrns(self, records):
        """
        convert unigetestbed ids to hrns
        """
    ...
    def fill_record_sfa_info(self, records):
        ...
        # get the unigetestbed records
        unigetestbed_user_list, unigetestbed_users = [], {}
        unigetestbed_user_list = self.shell.GetUsers({'user_ids': user_ids})
        unigetestbed_users = list_to_dict(unigetestbed_user_list, 'user_id')
    ...
    def testbed_name (self): return "unigetestbed"
    ...
    def perform_operational_action (self, urns, action, options={}):
        # unigetestbed doesn't support operational actions. Lets pretend like it
    ...
    def shutdown (self, xrn, options={}):
        xrn = unigetestbedXrn(xrn=xrn, type='slice')
